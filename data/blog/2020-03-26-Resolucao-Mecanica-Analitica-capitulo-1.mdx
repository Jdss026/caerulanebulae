...
\end{aligned}
$$
obtemos as derivadas parciais em $ x ~~ \text{e} ~~ \theta$:
$$
\begin{aligned}

\frac{\partial L}{\partial \dot{x}} &= m(\dot x+l \dot \theta cos \theta) & \frac{\partial L}{\partial \dot \theta} &= m(l^2\dot \theta+l\dot x cos \theta)\\

\frac{\partial L}{\partial x} &= -mkx & \frac{\partial L}{\partial \theta} &= ml\dot \theta sen\theta(-\dot \theta \dot x+g)\\
...
\frac{d}{dt}\left(m(\dot x+l \dot \theta cos \theta)\right) + mkx = 0 \\

m\left(\ddot x+l \ddot \theta cos \theta - l\dot \theta^2 sen \theta\right) + mkx = 0 \\

\ddot x+l \ddot \theta cos \theta - l\dot \theta^2 sen \theta + kx = 0
$$
...
m\frac{d}{dt}\left(l^2 \dot \theta+l\dot x cos \theta \right)-ml\dot \theta sen\theta(-\dot \theta \dot x+g) = 0 \\
$$
